...
export const fetchOffers = createAsyncThunk<OfferItemDto[], undefined, { extra: Extra }>(
  Action.FETCH_OFFERS,
  async (_, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<OfferItemDto[]>(ApiRoute.Offers);
...
export const fetchOffer = createAsyncThunk<OfferDto, Offer['id'], { extra: Extra }>(
  Action.FETCH_OFFER,
  async (id, { extra }) => {
...
export const registerUser = createAsyncThunk<void, UserRegister, { extra: Extra }>(
  ...
  async (userData, { extra }) => {
    const { user, avatar } = adaptSignupToServer(userData);
    const { api, history } = extra;
    ...
    await api.post<{ id: string }>(ApiRoute.Register, payload, {
